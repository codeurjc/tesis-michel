...
The most recognized state of the art in this field is the SZZ algorithm, which is based on identifying the change that fix the bug to analyze the lines that have been modified or deleted, assuming that the bug was introduced by the last change made in those lines before the fix. \patxi{Habla aquí de las limitaciones, porque luego las menciones justo en la siguiente frase}
...
The results obtained in this thesis show that \patxi{quizá enumerar estos puntos: a)...; b)...; c)...}compiling past snapshots is greatly affected by time unless mitigation measures are applied; 
