...
\begin{figure*}[ht!]
    ...
    \caption{Overview of testabilities - Each bar represents a project and the projects are ordered by the corresponding Testability Rate (A and T) of each project.\patxi{Hay que ver cómo decir esto que se entienda mejor}\michel{Done}}
...
\patxi{¿Por qué no aquí si estamos en la sección de análisis?}
\michel{To discuss: Es una buena pregunta. La razón de que esté en la discussion es que el "análisis" de las razones por las que algunos proyectos tienen una baja testability no es exhaustivo, no es un análisis real, si no no una discussion de los principales problemas que hemos detectado}
