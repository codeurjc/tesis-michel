...
\patxi{Yo aquí intentaría ligarlo con el capítulo anterior: As discussed in Chapter 3, to our knowledge...}To our knowledge, Tufano et al.~\cite{tufano2017there} presented the most complete study on the compilability of past snapshots.
...
However, we are not aware of studies that systematically analyzed to which extent tests present in past snapshots can be built, and run with a result of success\patxi{Aquí podríamos reforzar esto con algo como: thus leaving practitioners without a clue of whether the tests will still compile and pass, or which practices they should follow to guarantee that tests will still work in the future. Pero ojo: no damos esas mejores prácticas en el paper!!}. Leveraging on the previous research on compilability of past snapshots\patxi{Yo aquí diría, basándonos en el capítulo 3, indicando que extendemos la investigación para cubrir los tests}, in this chapter we present a study that extends it having into account if testing of those past snapshots is still possible. 
...
The method we followed is in summary as follows: for each snapshot of each project, we automatically build \patxi{¿debería ser compile? Porque has dicho que vas a usar compile en lugar de build.}its main code, its tests, and then we run tests whenever possible. 
