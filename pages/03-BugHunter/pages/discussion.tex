...
In this section we will discuss in detail the difficulties of transplanting a test into the past~\ref{sec:transplant-discuss} \patxi{No tengo muy claro qué aporta aquí poner las referencias de las subsubsecciones.}and the implications of our work for practitioners~\ref{sec:implications-practitioners} and researchers~\ref{sec:implications-practitioners}.
...
\begin{inparaenum}[\bf(1)]
    ...
    \item a comparison of our dataset (\datasetName) with a previous BICs dataset using Defects4J as bug dataset~\ref{sec:induce-benchmark}.\patxi{Esto no se entiende muy bien. A comparison of our dataset with a similar BIC dataset that was also using Defects4J as bug dataset?}
\end{inparaenum}
...
We find\patxi{found?} references to dependencies (not included in Defects4J) that include the suffix ``-SNAPSHOT'', which indicates that this is a volatile development version and is sometimes removed from the dependency repositories (causing the impossibility to compile a project that depends on them). 
...
\patxi{Separar en nuevo párrafo, porque es otro fix diferente}In 131 bugs from 6 projects we faced problems with source code parsing (due to the inclusion of unrecognized characters in strings or comments). 
...
This means that if there is a problem compiling just a single test, even if we do not have the intention to run it, because we only want to run the transplanted test, we cannot run it. \patxi{Yo diría más bien: the test compilation fails, and therefore we cannot run the transplanted test (even if it compiled successfully)}
...
An important result of our study is a dataset of BICs (\datasetName), automatically found from their BFC\patxi{(extracted from the Defects4J dataset)}, and manually validated, extracted from the Defects4J dataset. Based also on Defects4J, InduceBenchmark~\cite{wen2019exploring}, a dataset with 91 BICs, was created to evaluate SZZ implementations. \patxi{Quizá conviene dar un poco más de detalle: los 91 BICs son de los mismos proyectos que usamos nosotros (ya dices después que 9 son del que descartamos), por otro lado: 91 son pocos para los 800 bugs del dataset original, ¿qué bugs han seleccionado? Unos pocos de cada proyecto, sólo de algunos proyectos?}
