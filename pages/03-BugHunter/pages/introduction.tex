...
\patxi{Quizá el título podría ser (ya sé que el paper está lanzado con el actual): Bug localization trough regression testing}
...
% jgb: Proposal for substitutng the previous para
\patxi{Leveraging on the knowledge from compiling and running tests in past snapshots, as described in the two previous chapters, in this chapter we apply this knowledge and tooling to study...}
...
By the ``compilability problem'' we mean (a) the set of issues that prevent the source code of a snapshot of a software project from being built, and (b) the construction problems derived from compiling the transplanting code (in our case the regression test which will act as the perfect test) to previous versions. The (a) part of this problem has been addressed previously by researchers~\cite{tufano2017there} \patxi{and has been extended as part of this thesis} (as it has been discussed in Chapter~\ref{chapter:buildability} as well \patxi{si pones la frase que te comentom, quitaría el "as well"}). However, in our case we also need to study (b), i.e., the compilability of the regression test that has been transplanted into the commit under scrutiny. 
