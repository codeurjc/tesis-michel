...
% jgb: Proposal for substitutng the previous para
In this paper we study to which extent regression tests can be used as perfect tests. 
Regression tests~\cite{desikan2006softwareRegressionTesting,wahl1999overview} are written to avoid regressions~\footnote{In this paper, by ``regression'' we mean a change in the source code that breaks a functionality that was working properly.}, by detecting the reintroduction in future changes of an already fixed bug. 
...
which is the whole idea of spending effort in writing them. For the rest of this paper, we will assume that regression tests are perfect detectors, and we will focus on transplantability, which would be the only blocker to decide if they can be used. To transplant a test to a past snapshot, the regression test should be built (compiled, in the case of compiled languages), and run correctly (producing a ``fail'' or ``success'' output consistently).
...
% \textbf{RQ\textsubscript{0}: ``To which extent can tests be transplanted into the past?''} 
...
% \textbf{RQ\textsubscript{1}: ``In which cases can the BIC be found using regression tests?''}
...
\newcommand{\rqonea}[0]{RQ\textsubscript{1A}: ``How far can a test be transplanted into the past?''}
\textbf{\rqonea} 
...
\newcommand{\rqoneb}[0]{RQ\textsubscript{1B}: ``How compilability and runnability problems impact the transplantation of the regression tests to the past?''}
\textbf{\rqoneb}
...
\newcommand{\rqtwo}[0]{RQ\textsubscript{2}: ``Can the BIC for a given bug be found using its regression test?''}
\textbf{\rqtwo}
...
% \textbf{RQ\textsubscript{2}: ``To what extent can SZZ-like algorithm detect the BIC?''}
...
% \newcommand{\rqtwo}[0]{RQ\textsubscript{2}: ``How are the BICs\as{I am not sure that I understand "How are the BICs".}that SZZ derivatives are not able to detect?''}
% \textbf{\rqtwo}
...
The results of applying the methodology are reported in Section~\ref{sec:results}.
