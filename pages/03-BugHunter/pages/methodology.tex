...
\patxi{Pero ojo, aquí se usa sólo desde el BFC hasta n, no para todo el histórico de commits. NO es exactamente la misma.} 
\michel{Lo clarifico. En cualquier caso, al final la compilability siempre se calcula desde el commit inicial hasta otro (el último según cuando nos lo hayamos clonado o en este caso el BFC) }
...
\patxi{Ojo, decir aquí que esto es una aportación de la tesis y que en el trabajo original del perfect test no estaba}
\michel{Añadido}

With this in mind, we developed Algorithm~\ref{alg:bic} to detect the BIC in this graph. 
This algorithm receives as parameters the graph, annotated with the result of running the tests (its status, which can be success, fail or error), and the identification of the BFC in that graph, \patxi{and the algorithm returns ...}\michel{Quedan varios "and" seguidos}returning the list of candidates to be the BIC. 
...
For that, the algorithm finds the node $n$ in which the test fails, and in all precedent nodes to $n$ it succeeds (success parents), which would make $n$ a candidate to be the BIC.
...
\item \textbf{The test does not succeed in any precedent commit.}
  ...
  \patxi{¿No deberíamos distinguir ambos casos? No me parece lo mismo que el test no pase nunca, pero ejecute, que el hecho de que no se pueda ejecutar. En el primer caso puedes decir que el bug siempre ha estado ahí. En el segundo, no, porque hay commits para los que no sabes el resultado.}
  \michel{Yo los distingui (hablabamos de commits grises o negros), pero se acordo (no por mi parte) que los trataramos igual. Se habla largo y tendido de estos casos en la discussion}
\end{itemize}
